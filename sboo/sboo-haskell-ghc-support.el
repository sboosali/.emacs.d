...
(defvar sboo-haskell-ghc-supported-extensions
  '(
    ;;; BEGIN sboo-haskell-ghc-supported-extensions
    "AllowAmbiguousTypes"
    ...
    "Unsafe"
    "ViewPatterns"
    ;;; END sboo-haskell-ghc-supported-extensions
    )
  "List of language extensions supported by any known version of GHC.

This list should be updated by running `sboo-haskell-update-ghc-support'.")

(defvar sboo-haskell-ghc-supported-options
  '(
    ;;; BEGIN sboo-haskell-ghc-supported-options
    "-#include"
    "--abi-hash"
    "--frontend"
    "--help"
    ...
    "-w"
    "-with-rtsopts"
    ;;; END sboo-haskell-ghc-supported-options
    )
...
(defun sboo-haskell-update-ghc-support (ghc-path)
  "Update `sboo-haskell-ghc-supported-options' and `sboo-haskell-ghc-supported-extensions'.
...
Options and extension are kept in ascending order."
  ...
  (let ((extentions (split-string (shell-command-to-string (concat ghc-path " --supported-extensions"))))
    ...
    (with-current-buffer
        (find-file-noselect (replace-regexp-in-string "\\.elc$" ".el" (symbol-file 'sboo-haskell-ghc-supported-options)))
      (save-excursion
        (goto-char (point-min))
        (re-search-forward "BEGIN sboo-haskell-ghc-supported-extensions")
        (forward-line 1)
        (let ((point (point)))
          (re-search-forward "END sboo-haskell-ghc-supported-extensions")
          (goto-char (line-beginning-position))
          (delete-region point (point)))
        (setq sboo-haskell-ghc-supported-extensions
              (delete-dups (sort (append extentions sboo-haskell-ghc-supported-extensions) #'string<)))
        (dolist (item sboo-haskell-ghc-supported-extensions)
          (insert "    \"" item "\"\n"))
        (re-search-forward "BEGIN sboo-haskell-ghc-supported-options")
        (forward-line 1)
        (let ((point (point)))
          (re-search-forward "END sboo-haskell-ghc-supported-options")
          (goto-char (line-beginning-position))
          (delete-region point (point)))
        (setq sboo-haskell-ghc-supported-options
              (delete-dups (sort (append options sboo-haskell-ghc-supported-options) #'string<)))
        (dolist (item sboo-haskell-ghc-supported-options)
          (insert "    \"" item "\"\n"))))))

(provide 'sboo-haskell-ghc-support)
