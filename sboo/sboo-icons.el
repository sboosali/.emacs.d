;;; sboo-icons.el --- My icons -*- coding: utf-8; lexical-binding: t -*-
...
(defcustom sboo-icons-files-alist

  '(
    ;; programs:

    'haskell ?λ "lambda" 
    'elisp ?E "" 'emacs-lisp-mode
    'bash ? "shell" 'shell-script-mode 
    'nix ? "moon"
    'python ? "snake"
    'ruby ? "gem"
    ...
    expresso-mode "coffee" 
    )

  "AList that associates File Types with Unicode Icons.

the File Types:

• of Files (for Non-Buffers, e.g. in Dired),:

    • are guessed from the files's Path, in particular its File Extension and sometimes the Basename. c.f. `auto-mode-alist'.
    • are guessed from the files's First Line, in particular its “Shebang” (for Non-Buffers, e.g. in Dired). c.f. `auto-mode-alist'.

• of File-Buffers:

    • are guessed from the `major-mode', which was alreafy guessed indirectly via `auto-mode-alist', `auto-interpreter-alist', `magic-mode-alist', etc.
...
(defvar sboo-icons/alltheicons-alist
  '(
...
(defvar sboo-icons/material-icons-alist
  '(("3d_rotation"                                  . "\xe84d")
    ("ac_unit"                                      . "\xeb3b")
    ("access_alarm"                                 . "\xe190")
    ("access_alarms"                                . "\xe191")
